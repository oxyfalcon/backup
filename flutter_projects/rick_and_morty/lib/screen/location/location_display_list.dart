...
class DisplayLocation extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return CustomScrollView(
      ...
      slivers: [
        getList.when(
            onGoingError: (items, e, stk) => CurrentLocationWidget(list: items),
            onGoingLoading: (items) => CurrentLocationWidget(list: items),
            data: (list) {
              if (list.isEmpty) {
...
class CurrentLocationWidget extends StatelessWidget {
  const CurrentLocationWidget({super.key, required this.list});
